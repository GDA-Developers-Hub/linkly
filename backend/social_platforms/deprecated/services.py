...
class LinkedInOAuthManager(OAuthManager):
    ...
    def exchange_code_for_token(self, code):
        ...
        try:
            logger.info(f"Sending LinkedIn token exchange request to: {self.token_url}")
            response = requests.post(self.token_url, data=data)
            
            logger.info(f"LinkedIn token exchange response status: {response.status_code}")
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    error_details = response.text[:200] if response.text else "No error details available"
                    
                logger.error(f"LinkedIn error details: {error_details}")
                raise Exception(f"Failed to exchange code for token: {error_details}")
            
            token_data = response.json()
            logger.info(f"LinkedIn token exchange successful. Access token received.")
            return token_data
    ...
    def get_user_profile(self, access_token):
        ...
        logger.info("Starting LinkedIn profile retrieval")
        headers = {
            'Authorization': f'Bearer {access_token}',
            'X-Restli-Protocol-Version': '2.0.0'
        }
        
        # Generate a unique ID for this user based on current timestamp
        # This ensures we always have an ID even if we can't get the real one
        unique_id = f"linkedin_user_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        # Initialize profile data with default values
        # This will be returned if we only have w_member_social scope or encounter errors
        default_profile_data = {
            'id': unique_id,
            'first_name': 'LinkedIn',
            'last_name': 'User',
            'name': 'LinkedIn User',  # Add name field for consistency
            'username': f"linkedin_{unique_id[-8:]}",  # Add username for consistency
            'profile_picture_url': None,
            'email': None,
            'company_pages': [],
            'headline': 'LinkedIn Professional',
            'limited_access': True,  # Flag to indicate limited scope access
            'platform': 'linkedin',  # Add platform for filtering
            'picture': None  # Alternate field name for profile picture
        }
        
        try:
            logger.info(f"Fetching LinkedIn basic profile with access token starting with: {access_token[:10]}...")
            # Get basic profile
            ...
            if profile_response.status_code != 200:
                ...
                if profile_response.status_code == 403:
                    logger.warning("LinkedIn permissions error (possibly limited scopes). Using default profile data.")
                    return default_profile_data
                else:
                    logger.error(f"Failed to get LinkedIn profile: {profile_response.status_code} - {profile_response.text}")
                    # Instead of raising exception, return default profile to allow account creation
                    logger.warning("Returning default LinkedIn profile despite error to allow connection")
                    return default_profile_data
                
            # Successfully got profile data
            profile_data = profile_response.json()
            logger.info(f"Successfully retrieved LinkedIn profile data")
            
            # Ensure we have a valid ID
            if 'id' not in profile_data:
                profile_data['id'] = unique_id
                logger.warning(f"LinkedIn profile missing ID, using generated ID: {unique_id}")
            ...
            try:
                ...
                logger.info(f"Fetching LinkedIn profile picture")
                picture_response = requests.get(picture_url, headers=headers)
                
                if picture_response.status_code == 200:
                    picture_data = picture_response.json()
                    logger.info(f"Successfully retrieved LinkedIn profile picture")
                else:
                    logger.warning(f"Could not fetch LinkedIn profile picture: {picture_response.status_code}")
            except Exception as e:
                logger.warning(f"Error fetching LinkedIn profile picture: {str(e)}")
                # Continue despite picture retrieval failure
                
            # Attempt to get email address if we have access
            email = None
            try:
                email_url = f"{self.api_base_url}/emailAddress?q=members&projection=(elements*(handle~))"
                logger.info(f"Fetching LinkedIn email address")
                ...
                if email_response.status_code == 200:
                    ...
                    if elements and 'handle~' in elements[0]:
                        ...
                        logger.info(f"Successfully retrieved LinkedIn email")
                    else:
                        logger.warning("LinkedIn email address not found in response structure")
                else:
                    logger.warning(f"Could not fetch LinkedIn email (status {email_response.status_code})")
            except Exception as e:
                logger.warning(f"Error fetching LinkedIn email: {str(e)}")
                # Continue despite email retrieval failure
            ...
            result = {
                'id': profile_data.get('id', unique_id),
                'first_name': profile_data.get('localizedFirstName', 'LinkedIn'),
                'last_name': profile_data.get('localizedLastName', 'User'),
                'name': f"{profile_data.get('localizedFirstName', 'LinkedIn')} {profile_data.get('localizedLastName', 'User')}".strip(),
                'headline': profile_data.get('headline', 'LinkedIn Professional'),
                'industry': profile_data.get('industryName'),
                'profile_picture_url': profile_picture_url,
                'picture': profile_picture_url,  # Add alternate field name for consistency
                'email': email,
                'company_pages': company_pages,
                'platform': 'linkedin',  # Add platform identifier for filtering
                'username': profile_data.get('vanityName', f"linkedin_{unique_id[-8:]}"),  # Add username field
                'account_type': 'profile'  # Specify account type
            }
            
            logger.info(f"Successfully processed LinkedIn profile data for user ID: {result['id']}")
            return result
...
def connect_social_account(user, platform_name, auth_code):
    ...
    logger.info(f"Starting connection process for platform: {platform_name}")
    try:
        oauth_manager = get_oauth_manager(platform_name)
        logger.info(f"OAuth manager initialized for {platform_name}")
        
        # Exchange code for token
        logger.info(f"Exchanging auth code for {platform_name} access token")
        ...
        logger.info(f"Retrieving user profile for {platform_name}")
        profile_data = oauth_manager.get_user_profile(processed_token['access_token'])
        logger.info(f"Successfully retrieved profile data for {platform_name}")
