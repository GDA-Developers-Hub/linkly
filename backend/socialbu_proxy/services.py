...
class SocialBuService:
    ...
    # User Management
    # Note: We're not registering users with SocialBu, only getting tokens
    ...
    #     
    #     # Check if the endpoint is correct for registration
    #     result = self.make_request('/users/register', 'POST', {
    #         'email': email,
    #         'password': password,
    #         'name': name
    #     })
    #     
    #     # After registration, get an auth token
    #     auth_result = self.authenticate(email, password)
    #     logger.info(f"SocialBu registration successful, retrieved auth token")
    #     
    #     return auth_result
