...
const buildUrl = (endpoint: string): string => {
  ...
  const endpointWithSlash = cleanEndpoint.endsWith("/")
    ? cleanEndpoint
    : `${cleanEndpoint}/`;
...
class API {
  ...
  private getHeaders(endpoint: string = ""): HeadersInit {
    ...
    if (!this.isPublicEndpoint(endpoint) && this.accessToken) {
      ...
      console.log(
        `Adding Authorization header: Bearer ${this.accessToken.substring(
          0,
          10
        )}...`
      );
  ...
  private async refreshAccessToken(): Promise<boolean> {
    ...
    try {
      const response = await fetch(buildUrl("accounts/token/refresh/"), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    if (!response.ok) {
      ...
      if (contentType && contentType.includes("application/json")) {
        try {
          ...
          console.error(
            `Error response text: ${text.substring(0, 200)}${
              text.length > 200 ? "..." : ""
            }`
          );
        ...
        const text = await response.text();
        console.error(
          `Received non-JSON response from ${url}. First 200 chars: ${text.substring(
            0,
            200
          )}`
        );
      ...
      if (
        endpoint.includes("generate-caption") ||
        endpoint.includes("generate-hashtags")
      ) {
        if (response.status === 500) {
          errorMessage =
            "AI generation service is currently unavailable. This may be due to missing API keys or service disruption.";
        }
      }
  ...
  async updateProfile(data: Partial<User>): Promise<User> {
    return this.request<User>("/account/profile/", "PUT", data);
  }
  ...
  //Posts
  async createPost(posts: Post): Promise<PostResponse> {
  ...
  async getPlans(): Promise<Plan[]> {
    const response = await this.request<PaginatedResponse<Plan>>(
  ...
  async createSubscription(
    planId: number,
    paymentMethodId: string = "dummy-payment"
  ...
  async generateCaption(
    ...
    return this.request<{ caption: Caption; tokens_used: number }>(
  ...
  async getSavedCaptions(): Promise<Caption[]> {
    return this.request<Caption[]>("/api/content/saved-captions/");
  }

  async saveCaption(
    caption: Omit<Caption, "id" | "date_created">
  ): Promise<Caption> {
    return this.request<Caption>(
      "/api/content/saved-captions/",
      "POST",
      caption
    );
  }
  ...
  async getTrendingHashtags(platform = "all", limit = 20): Promise<Hashtag[]> {
    return this.request<Hashtag[]>(
      `/api/content/hashtags/trending/?platform=${platform}&limit=${limit}`
    );
  }
  ...
  async getRelatedHashtags(
    ...
    return this.request<Hashtag[]>(
      `/api/content/hashtags/related/?hashtag=${hashtag}&platform=${platform}&limit=${limit}`
    );
  }
  ...
  async generateHashtags(
    ...
    return this.request<{ hashtags: Hashtag[] }>(
  ...
  async getSavedHashtagGroups(): Promise<HashtagGroup[]> {
    return this.request<HashtagGroup[]>("/api/content/hashtag-groups/");
  }

  async saveHashtagGroup(
    group: Omit<HashtagGroup, "id">
    ...
    return this.request<HashtagGroup>(
      "/api/content/hashtag-groups/",
      "POST",
      payload
    );
  }
  ...
  async getAccounts(): Promise<SocialMediaAccount[]> {
    return this.request<SocialMediaAccount[]>("accounts/");
  }
