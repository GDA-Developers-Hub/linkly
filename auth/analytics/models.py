...
class AccountMetrics(models.Model):
    ...
    social_account = models.ForeignKey(SocialAccount, on_delete=models.CASCADE, related_name='metrics')
    ...
    def __str__(self):
        return f"Metrics for {self.social_account} on {self.date}"
...
class AudienceInsight(models.Model):
    ...
    def __str__(self):
        return f"Audience insight for {self.social_account} on {self.date}"
...
class BestTimeToPost(models.Model):
    ...
    social_account = models.ForeignKey(SocialAccount, on_delete=models.CASCADE, related_name='best_times')
    ...
    def __str__(self):
        return f"Best time for {self.social_account}: {self.day_of_week} at {self.hour}:00"
